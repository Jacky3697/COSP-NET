...
def log_dict(writer, scalars, step, prefix):
    for k, v in scalars.items():
        if v is None:
            continue
        try:
            writer.add_scalar(prefix + "/" + k, float(v), step)
        except Exception as e:
            print(f"[warn] skip tensorboard log {prefix}/{k}: {v}, {e}")
...
def compute_loss(model, batch, losses, loss_funcs, pa):
    ...
    if "camo_align" in losses:
        if 'camo_attention_map' not in logits:
            raise KeyError(
                "camo_align loss requires Data.use_camo_alignment=true and "
                "Data.use_camo_module=true")
        non_term_mask = torch.logical_not(is_last)
        pred_camo_map = logits['camo_attention_map'].squeeze(1)
        tgt_fix_map = batch['target_fix_map'].to(device)
        if pred_camo_map.size(-1) != pa.im_w:
            pred_camo_map = F.interpolate(
                pred_camo_map.unsqueeze(1),
                size=(pa.im_h, pa.im_w),
                mode='bilinear',
                align_corners=False,
            ).squeeze(1)
        loss_dict['camo_align'] = loss_funcs['camo_align'](
            pred_camo_map[non_term_mask],
            tgt_fix_map[non_term_mask])
...
if __name__ == '__main__':
    ...
    best_score = -float("inf") # best_score = 0
    log_dir = hparams.Train.log_dir
    if args.eval_only:
        ckpt = torch.load(os.path.join(log_dir, "best_model.pt"), map_location=device, weights_only=False)

        if isinstance(model, torch.nn.DataParallel):
            model.module.load_state_dict(ckpt['model'])
        else:
            model.load_state_dict(ckpt['model'])
        print("✅ Loaded best_model.pt for evaluation")
        run_evaluation()
        ...
        loss_weights = {
            ...
            "camo_align": getattr(hparams.Train, "camo_align_weight", 0.05),
        }
            ...
            for i_batch, batch in enumerate(train_gaze_loader):
                ...
                if global_step % print_every == print_every - 1:
                    ...
                    writer.add_scalar('train/lr',
                                      ...
                                      global_step)
                    model_for_log = model.module if isinstance(model, torch.nn.DataParallel) else model
                    if hasattr(model_for_log, "uncertainty_alpha") and model_for_log.uncertainty_alpha is not None:
                        alpha = model_for_log.uncertainty_alpha_scale * torch.tanh(
                            model_for_log.uncertainty_alpha.detach()
                        )
                        writer.add_scalar('train/uncertainty_alpha',
                                          float(alpha.item()),
                                          global_step)
                    for k in loss_dict_avg:
                        loss_dict_avg[k] = 0
                ...
                if global_step % eval_every == eval_every - 1:
                    if args.pretrain:
                        ...
                        score = rst_fv.get("Greedy_FV_cNSS", None)
                        if score is not None:
                            if score > best_score:
                                ...
                                # 拼接文件路径
                                file_path = os.path.join(log_dir, "best_model.pt")
                                print(f"Saving model to {file_path}")  # 打印出路径以确认
                                torch.save(save_obj, file_path)
                                print(f"🌟 New Best Model Found! Score={best_score:.4f} -> Saved to best_model.pt")
