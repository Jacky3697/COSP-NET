...
from .cosp_model import CamouflagePerceptionModule
...
class ImageFeatureEncoder(nn.Module):
    def __init__(self,
        ...
        # 兼容 pkl 格式
        if isinstance(bb_weights, dict) and "model" in bb_weights:
            bb_weights = bb_weights["model"]
...
class HumanAttnTransformer(nn.Module):
    def __init__(
        ...
        self.use_camo_module = getattr(pa, "use_camo_module", False)
        self.use_uncertainty_fusion = getattr(pa, "use_uncertainty_fusion", True)
        self.use_camo_alignment = getattr(pa, "use_camo_alignment", False)
        self.use_camo_alignment_gate = getattr(pa, "use_camo_alignment_gate", False)
        self.camo_alignment_gate_alpha = float(
            getattr(pa, "camo_alignment_gate_alpha", 0.05))
        self.camo_fusion_alpha = float(getattr(pa, "camo_fusion_alpha", 0.1))
        if self.use_camo_module:
            print("Initializing Camouflage Perception Module (SINet-V2)...")
            self.camo_module = CamouflagePerceptionModule(hidden_dim=featmap_channels)
            self.camo_norm = LayerNorm2d(featmap_channels)
            if self.use_camo_alignment:
                self.camo_attention_head = nn.Conv2d(featmap_channels, 1, kernel_size=1)
                if self.use_camo_alignment_gate:
                    nn.init.zeros_(self.camo_attention_head.weight)
                    nn.init.zeros_(self.camo_attention_head.bias)
            else:
                self.camo_attention_head = None
        else:
            self.camo_module = None
            self.camo_norm = None
            self.camo_attention_head = None
        if self.use_uncertainty_fusion:
            self.uncertainty_alpha = nn.Parameter(torch.zeros(1))
            self.uncertainty_alpha_scale = float(
                getattr(pa, "uncertainty_alpha_scale", 0.1)
            )
        else:
            self.uncertainty_alpha = None
            self.uncertainty_alpha_scale = 0.0
    ...
    def _fuse_camo_features(self, img, img_embs_s2, return_attention=False):
        if self.camo_module is None:
            if return_attention:
                return img_embs_s2, None, None
            return img_embs_s2

        camo_emb, uncertainty_map = self.camo_module(img)
        camo_prior_map = uncertainty_map
        camo_emb = F.interpolate(
            camo_emb,
            size=img_embs_s2.shape[-2:],
            mode="bilinear",
            align_corners=False,
        )
        camo_emb = self.camo_norm(camo_emb)
        img_embs_s2 = img_embs_s2 + self.camo_fusion_alpha * camo_emb
        ...
        if self.use_uncertainty_fusion:
            ...
            alpha = self.uncertainty_alpha_scale * torch.tanh(self.uncertainty_alpha)
            img_embs_s2 = img_embs_s2 * (1 + alpha * uncertainty_map)

        camo_attention = None
        if self.camo_attention_head is not None:
            camo_attention = self.camo_attention_head(camo_emb)
            if self.use_camo_alignment_gate:
                camo_gate = 2 * torch.sigmoid(camo_attention) - 1
                img_embs_s2 = img_embs_s2 * (
                    1 + self.camo_alignment_gate_alpha * camo_gate)

        if return_attention:
            return img_embs_s2, camo_attention, camo_prior_map
        return img_embs_s2
    ...
    def forward(self,
                ...
                return_camo_maps=False):

        # Prepare dorsal embeddings
        img_embs_s4, img_embs_s1, img_embs_s2 = self.encoder(img)
        img_embs_s2, camo_attention_map, camo_prior_map = self._fuse_camo_features(
            img, img_embs_s2, return_attention=True)
        ...
        if (self.training or return_camo_maps) and camo_attention_map is not None:
            out['camo_attention_map'] = camo_attention_map
        if return_camo_maps and camo_prior_map is not None:
            out['camo_prior_map'] = camo_prior_map
        return out

    def encode(self, img: torch.Tensor):
        # Prepare dorsal embeddings
        img_embs_s4, img_embs_s1, img_embs_s2 = self.encoder(img)
        img_embs_s2 = self._fuse_camo_features(img, img_embs_s2)
        high_res_featmaps = self.input_proj(img_embs_s4)
        if self.output_feature_map_name == 'P4':
            output_featmaps = high_res_featmaps
        elif self.output_feature_map_name == 'P2':
