...
def build(hparams, dataset_root, device, is_pretraining, is_eval=False, split=1):
    ...
    if len(hparams.Model.checkpoint) > 0:
        ...
        ckp_path = hparams.Model.checkpoint
        if not os.path.isabs(ckp_path):
            ckp_path = join(hparams.Train.log_dir, ckp_path)
        ...
        if hparams.Train.transfer_learn == 'none' or hparams.Train.resume_training:
            ...
            if hparams.Train.freeze_trained_params:
                for name, param in model.named_parameters():
                    if 'camo_attention_head' in name:
                        param.requires_grad = True
                    elif 'fix_ind_emb' not in name and name in ckp['model']:
    ...
    sps_test_fv = list(
        filter(lambda x: x['split'] == 'valid', human_scanpaths_fv))
